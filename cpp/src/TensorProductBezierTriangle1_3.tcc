...
template<typename T>
using TPBT1_3 = TensorProductBezierTriangle<T, 1, 3>;

template<typename T>
template<int D>
std::array<TPBT1_3<T>, 4> TPBT1_3<T>::split() const
{
...
template<typename T>
const typename TPBT1_3<T>::DomainPoints& TPBT1_3<T>::domainPoints()
{
...
template<typename T>
typename TPBT1_3<T>::Basis TPBT1_3<T>::makeBasis(const Coords& pos)
{
    return (Basis{} <<
...
typename TPBT1_3<T>::Coeffs
TPBT1_3<T>::splitCoeffs(const Coeffs& in)
{
...
template <typename T>
typename TPBT1_3<T>::Coeffs
TPBT1_3<T>::computeCoeffs(const Coeffs& in)
{
    auto out = Coeffs{};
    out[0] = in[0];
